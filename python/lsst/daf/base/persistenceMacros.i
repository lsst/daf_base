...
// dynamic down casts from boost::shared_ptr<lsst::daf::base::Persistable>
%define %lsst_persistable_shared_ptr(PtrName, CppType...)
    ...
    %extend CppType {
        #include <stdexcept>
        static CppType* swigConvert(lsst::daf::base::Persistable* p) {
            if (!p) {
                throw std::runtime_error("Cannot convert null Persistable pointer");
            }
            CppType* ptr = dynamic_cast<CppType*>(p);
            if (!ptr) {
                throw std::runtime_error("Persistable was not of the expected type associated with smart pointer " # PtrName);
            }
            return ptr;
        }
    }

    // Convert boost::shared_ptr<lsst::daf::base::Persistable> arguments to boost::shared_ptr<CppType >
    %typemap(in) boost::shared_ptr<CppType > const & (boost::shared_ptr<CppType > temp) {
        void* argp = 0;
        int res = SWIG_ConvertPtr($input, &argp, $descriptor(boost::shared_ptr<lsst::daf::base::Persistable> *), 0);
        if (SWIG_IsOK(res)) {
            temp = boost::dynamic_pointer_cast<CppType >(*
                reinterpret_cast<boost::shared_ptr<lsst::daf::base::Persistable> *>(argp)
            );
            if (!temp) {
    ...
    // Convert lsst::daf::base::Persistable* arguments to CppType*
    %typemap(in) CppType* (CppType* temp) {
        void* argp = 0;
        int res = SWIG_ConvertPtr($input, &argp, $descriptor(lsst::daf::base::Persistable*), 0);
        if (SWIG_IsOK(res)) {
            temp = dynamic_cast<CppType*>(reinterpret_cast<lsst::daf::base::Persistable*>(argp));
            if (!temp) {
    ...
    %typemap(in) CppType* DISOWN (CppType* temp) {
        void* argp = 0;
        int res = SWIG_ConvertPtr($input, &argp, $descriptor(lsst::daf::base::Persistable*), SWIG_POINTER_DISOWN);
        if (SWIG_IsOK(res)) {
            temp = dynamic_cast<CppType*>(reinterpret_cast<lsst::daf::base::Persistable*>(argp));
            if (!temp) {
                SWIG_exception_fail(SWIG_TypeError, "bad dynamic_cast from Persistable*");
    ...
    // To get SWIG to dispatch boost::shared_ptr<lsst::daf::base::Persistable> properly,
    ...
    %typecheck(SWIG_TYPECHECK_POINTER) boost::shared_ptr<CppType > const & {
        void* ptr = 0;
        if (SWIG_IsOK(SWIG_ConvertPtr($input, &ptr, $descriptor(boost::shared_ptr<lsst::daf::base::Persistable> *), 0))) {
            $1 = 1;
    ...
    // To get SWIG to dispatch lsst::daf::base::Persistable* properly,
    // we have to pretend it has the same type as CppType*
    %typecheck(SWIG_TYPECHECK_POINTER) CppType* {
        void* ptr = 0;
        if (SWIG_IsOK(SWIG_ConvertPtr($input, &ptr, $descriptor(lsst::daf::base::Persistable*), 0))) {
            $1 = 1;
        } else if (SWIG_IsOK(SWIG_ConvertPtr($input, &ptr, $1_descriptor, 0))) {
            $1 = 1;
        } else {
